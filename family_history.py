...
def count_genders(people_dict):
    ...
    females = 0
    for _, values in people_dict.items():
        if values[GENDER_INDEX].lower() == 'm':
            males += 1
        elif values[GENDER_INDEX].lower() == 'f':
            females += 1
    print(f"Males are: {males}\nFemales are: {females}")
    pass
...
def print_marriages(marriages_dict, people_dict):
    ...
    for _, values in marriages_dict.items():
        wedding_date = values[WEDDING_YEAR_INDEX]

        husband = values[HUSBAND_KEY_INDEX]
        h_value = people_dict[husband]
        h_name = h_value[NAME_INDEX]
        h_age = wedding_date - h_value[BIRTH_YEAR_INDEX]

        wife = values[WIFE_KEY_INDEX]
        w_value = people_dict[wife]
        w_name = w_value[NAME_INDEX]
        w_age = wedding_date - w_value[BIRTH_YEAR_INDEX]
        
        print(f"""{h_name} {h_age} > {wedding_date} < {w_name} {w_age}""")

def count_marriages(marriage_dict, people_dict):
    for key, p_value in people_dict.items():
        count = 0
        for _, m_value in marriage_dict.items():
            h_index = m_value[HUSBAND_KEY_INDEX]
            w_index = m_value[WIFE_KEY_INDEX]
            if key == (h_index or w_index):
                print(key)
        #         key = p_value[NAME_INDEX]
        #         count += 1
        # print(f"{key} has been married {count} times")
