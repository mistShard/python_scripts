...
def test_suffix():
    ...
    # call the suffix function an verify that it returns a string.
    assert isinstance(suffix("test", "please"), str)

    # Call the suffix function nine times and use anasser
    # statement to verify that the string returned by the 
    # suffix function is correct each time.
    assert suffix("", "") == ""
    assert suffix("", "correct") == ""
    assert suffix("clear", "angelic") == ""
