...
def main():
    ...
    formula = input("Input the molecular formula of the sample: ")

    # Get the mass of a chemical sample in grams from the user.
    sample_mass = float(input("Input the mass in grams of the sample: "))
    ...
    # Call the known_molecules function and store the known molecules
    # dictionary in a variable
    known_formulas_dict = known_molecules()

    # Call the get_formula_name function to get the formula name and
    # print its corresponding common name
    formula_common_name = get_formula_name(formula, known_formulas_dict)

    # Print the molar mass.
    print(f"The molar mass for {formula} is {round(molar_mass, 5)} grams/mole")

    # Print the number of moles.
    print(f"The number of moles in {formula} is {round(number_of_moles, 5)} moles")

    # Print the formula's common name
    if formula in known_formulas_dict:
        print(f"The common name for the formula - {formula} - \
is: {formula_common_name}")
    else:
        print(f"Sorry, the common name for the formula - {formula} - \
was not found")
...
def make_periodic_table():
    ...
    NAME_INDEX = 1
    ATOMIC_MASS_INDEX = 2
    periodic_table_dict = {element[SYMBOL_INDEX]:[element[NAME_INDEX], element[ATOMIC_MASS_INDEX]] for element in periodic_table}
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for inner_list in symbol_quantity_list:
        ...
        symbol_value = periodic_table_dict.get(symbol)
        atomic_mass = symbol_value[ATOMIC_MASS_INDEX]
...
def known_molecules():
    """
    This functions creates a dictionary containing some known chemical
    formulas and their common names. The key is the chemical formula and
    associated value is the common name
    
    Parameters:
    It contains no parameters
    
    returns:
    A dictionay containing chemical formula's as the key
    and common name as the value"""
    ...
    known_formulas_dict ={
        ...
        "H2O": "water",
        "NaHCO3": "baking soda",
        "NaOCl": "antiformin/chloride of soda",
        "NaClO": "bleach",
        "CHCl3": "coloroform",
...
def get_formula_name(formula, known_molecules_dict):
    ...
    """
    common_name = known_molecules_dict.get(formula)
    return common_name
